...
export class BookTradeComponent implements OnInit {
  ...
  onBook(f: NgForm) {
    ...
    this.dialogservice
      .OpenConfirmDialog('Are you sure to book?')
      .afterClosed()
      .subscribe((res) => {
        if (res) {
          this.isRateEnable = true;
          if (
            ...
            let obj = {
              ...
              tranferRate:
                f.value.tranferRate == 0 ? this.isRate : f.value.tranferRate,
              transferAmount: Number(
                f.value.transferAmount *
                  (f.value.tranferRate == 0 ? 66.0 : f.value.tranferRate)
        ...
        } else this.toaster.warning('Trade Not Booked');
      });
  }
}
